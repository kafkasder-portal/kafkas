...
CREATE TABLE IF NOT EXISTS users (
  ...
  type VARCHAR(50) DEFAULT 'individual' CHECK (type IN ('individual', 'organization', 'admin', 'volunteer')),
...
CREATE TABLE IF NOT EXISTS tasks (
  ...
  description TEXT,
  type VARCHAR(50) DEFAULT 'general' CHECK (type IN ('administrative', 'field_work', 'coordination', 'general')),
...
CREATE TABLE IF NOT EXISTS beneficiaries (
  ...
  type VARCHAR(50) DEFAULT 'individual' CHECK (type IN ('individual', 'family', 'organization', 'community')),
...
CREATE TABLE IF NOT EXISTS meetings (
  ...
  location VARCHAR(255),
  type VARCHAR(50) DEFAULT 'general' CHECK (type IN ('board', 'committee', 'general', 'volunteer')),
...
-- Messages tablosu (recipient_id removed as it's handled by message_recipients table)
CREATE TABLE IF NOT EXISTS messages (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  sender_id UUID REFERENCES users(id),
  subject VARCHAR(255),
  content TEXT NOT NULL,
  type VARCHAR(50) DEFAULT 'general' CHECK (type IN ('notification', 'announcement', 'personal', 'general')),
...
INSERT INTO users (email, password_hash, first_name, last_name, role, type, status) VALUES
('admin@kafportal.com', '$2b$10$example_hash', 'Admin', 'User', 'admin', 'individual', 'active')
ON CONFLICT (email) DO NOTHING;

-- Örnek kategoriler için inventory
INSERT INTO inventory (name, description, category, type, quantity, unit, status, location) VALUES
('Pirinç', '5kg paket pirinç', 'Gıda', 'general', 50, 'paket', 'available', 'Depo A'),
('Makarna', '500g paket makarna', 'Gıda', 'general', 100, 'paket', 'available', 'Depo A'),
('Süt', '1L süt', 'Gıda', 'general', 30, 'adet', 'low_stock', 'Depo B'),
('Kalem', 'Mavi kalem', 'Kırtasiye', 'general', 200, 'adet', 'available', 'Depo C')
ON CONFLICT DO NOTHING;

-- Örnek görevler
INSERT INTO tasks (title, description, type, status, priority, due_date) VALUES
('Depo kontrolü', 'Haftalık depo kontrolü yapılacak', 'general', 'pending', 'medium', NOW() + INTERVAL '7 days'),
('Bağış toplama', 'Mahalle bağış toplama kampanyası', 'general', 'pending', 'high', NOW() + INTERVAL '3 days'),
('Toplantı hazırlığı', 'Aylık yönetim kurulu toplantısı hazırlığı', 'general', 'pending', 'medium', NOW() + INTERVAL '5 days')
ON CONFLICT DO NOTHING;
