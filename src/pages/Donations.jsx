  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h2 style={{ fontSize: '1.5rem', fontWeight: '600', color: '#1a202c', margin: 0 }}>Bağış Listesi</h2>
          <p style={{ color: '#64748b', margin: '0.5rem 0 0 0' }}>Tüm bağış kayıtları ve detayları</p>
        </div>
        <motion.button
          style={{
            display: 'flex',
      ...
      <div className="card">
        ...
        <div style={{ overflowX: 'auto' }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '3rem', color: '#64748b' }}>
              {searchTerm ? 'Arama kriterlerine uygun bağış bulunamadı.' : 'Henüz bağış kaydı bulunmuyor.'}
            </div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid #e2e8f0' }}>
                  <th style={{ padding: '1rem', textAlign: 'left', fontWeight: '600', color: '#374151' }}>Bağışçı</th>
                  <th style={{ padding: '1rem', textAlign: 'left', fontWeight: '600', color: '#374151' }}>Tutar</th>
                  <th style={{ padding: '1rem', textAlign: 'left', fontWeight: '600', color: '#374151' }}>Tarih</th>
                  <th style={{ padding: '1rem', textAlign: 'left', fontWeight: '600', color: '#374151' }}>Tür</th>
                      ...
                      <td style={{ padding: '1rem', fontWeight: '500' }}>
                        <div>
                          ...
                          {donation.donor_email && (
                            <div style={{ fontSize: '0.8rem', color: '#64748b' }}>{donation.donor_email}</div>
                          )}
                        </div>
                      </td>
...
const DonationsVault = () => {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.5 }}
    >
      <h2 style={{ fontSize: '1.5rem', fontWeight: '600', color: '#1a202c', marginBottom: '1rem' }}>Bağış Veznesi</h2>
      <p style={{ color: '#64748b', marginBottom: '2rem' }}>Bağış kasası yönetimi ve takibi</p>
          ...
          {vaultStats.map((stat, index) => (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                ...
                <span style={{ fontSize: '0.8rem', color: stat.color, fontWeight: '600' }}>{stat.change}</span>
              </div>
              <div style={{ fontSize: '1.8rem', fontWeight: '700', color: '#1e293b' }}>{stat.value}</div>
            </motion.div>
          ))}
        </div>
      )}
...
const DonationsInstitutions = () => (
  <motion.div
    ...
    transition={{ duration: 0.5 }}
  >
    <h2 style={{ fontSize: '1.5rem', fontWeight: '600', color: '#1a202c', marginBottom: '1rem' }}>Kurumlar</h2>
    <p style={{ color: '#64748b', marginBottom: '2rem' }}>Bağışçı kurum yönetimi</p>
    
    <div className="card">
      <p style={{ textAlign: 'center', color: '#64748b', padding: '2rem' }}>Kurum listesi burada görüntülenecek...</p>
    </div>
  </motion.div>
)
    ...
    <div className="page-container">
      <div className="page-header">
        ...
        <div className="breadcrumb">
          ...
          <span>{navItems.find(item => item.path === location.pathname)?.label || 'Bağış Listesi'}</span>
        </div>
      </div>
