...
const PerformanceDashboard = () => {
  ...
  const { resourceMetrics, measureResourcePerformance } = useResourcePerformance()

  // Mock data for demonstration
  const [performanceHistory, setPerformanceHistory] = useState([])
  const [largeListData] = useState(Array.from({ length: 10000 }, (_, i) => ({
    id: i,
    name: `Item ${i}`,
  ...
  const renderTimeStatus = getPerformanceStatus(performance.metrics.renderTime, 16, 'lower')
  const memoryStatus = getPerformanceStatus(performance.metrics.memoryUsage, 80, 'lower')
      ...
      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className={`p-2 rounded-full ${renderTimeStatus.color.replace('text-', 'bg-')} bg-opacity-10`}>
                  <renderTimeStatus.icon className="h-6 w-6" />
                </div>
              </div>
              <div className="mt-2">
                <span className={`text-xs font-medium ${
                  performance.metrics.renderTime <= 16 ? 'text-green-600' : 'text-yellow-600'
                }`}>
                  {performance.metrics.renderTime <= 16 ? 'Optimal' : 'Needs optimization'}
                </span>
              </div>
            </motion.div>
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className={`p-2 rounded-full ${memoryStatus.color.replace('text-', 'bg-')} bg-opacity-10`}>
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    className={`h-2 rounded-full transition-all duration-300 ${
                      performance.metrics.memoryUsage > 80 ? 'bg-yellow-500' : 'bg-green-500'
                    }`}
                    style={{ width: `${Math.min(performance.metrics.memoryUsage, 100)}%` }}
                  />
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className={`p-2 rounded-full ${fpsStatus.color.replace('text-', 'bg-')} bg-opacity-10`}>
                  <fpsStatus.icon className="h-6 w-6" />
                </div>
              </div>
              <div className="mt-2">
                <span className={`text-xs font-medium ${
                  performance.metrics.fps >= 30 ? 'text-green-600' : 'text-yellow-600'
                }`}>
        ...
        {activeTab === 'metrics' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Resource Loading</h3>
                <div className="space-y-3">
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">Total Resources:</span>
                    <span className="font-medium">{resourceMetrics.totalResources}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">Loaded:</span>
                    <span className="font-medium text-green-600">{resourceMetrics.loadedResources}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">Failed:</span>
                    <span className="font-medium text-red-600">{resourceMetrics.failedResources}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">Total Size:</span>
                    <span className="font-medium">{(resourceMetrics.totalSize / 1024 / 1024).toFixed(2)} MB</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{networkMetrics.latency.toFixed(0)}ms</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">Bandwidth:</span>
                    <span className="font-medium">{networkMetrics.bandwidth.toFixed(1)} Mbps</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">Connection:</span>
                    <span className="font-medium capitalize">{networkMetrics.connectionType}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        {activeTab === 'optimization' && (
          <div className="space-y-6">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              ...
              <div className="space-y-3">
                {performance.getOptimizationSuggestions().map((suggestion, index) => (
                  <motion.div
                    key={index}
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Performance Warnings</h3>
        ...
        {activeTab === 'testing' && (
          <div className="space-y-6">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Virtual Scrolling Test (10,000 items)</h3>
              <VirtualList
                items={largeListData}
                itemHeight={80}
                containerHeight={400}
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Image Optimization Test</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <h4 className="text-sm font-medium text-gray-700 mb-2">Lazy Loaded</h4>
                  <OptimizedImage
                    src="https://picsum.photos/300/200"
