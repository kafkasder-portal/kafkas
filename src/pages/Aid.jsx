...
import { Calendar, Filter, Heart, MapPin, Package, Plus, Search, Stethoscope, Truck, Users } from 'lucide-react'
...
const Aid = () => {
  ...
  const loadAidPrograms = async () => {
    try {
      ...
      const activePrograms = data.filter(program => program.status === 'Aktif').length
      const beneficiaries = data.reduce((sum, program) => sum + program.beneficiaries, 0)
      const distributionPoints = new Set(data.map(program => program.location)).size
  ...
  // Arama filtresi
  useEffect(() => {
    const filtered = aidPrograms.filter(program =>
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h1 style={{ fontSize: '2rem', fontWeight: '700', color: '#1a202c', margin: 0 }}>Yardım Programları</h1>
          <p style={{ color: '#64748b', margin: '0.5rem 0 0 0' }}>İnsani yardım ve destek programları</p>
        </div>
      ...
      {/* Program Listesi */}
      <div className="card">
        <h3 style={{ fontSize: '1.25rem', fontWeight: '600', color: '#1a202c', marginBottom: '1.5rem' }}>Aktif Programlar</h3>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <span style={{ color: '#64748b', fontSize: '0.875rem' }}>Bütçe:</span>
                    <span style={{ fontWeight: '600', color: '#1a202c' }}>₺{program.budget.toLocaleString()}</span>
                  </div>
                </div>
                <span
                  style={{
