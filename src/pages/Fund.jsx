...
import { DollarSign, TrendingUp, Target, PieChart, Plus, Search, Filter, Calendar, Users } from 'lucide-react'
...
const Fund = () => {
  ...
  const loadFunds = async () => {
    try {
      ...
      const activeCampaigns = data.filter(fund => fund.status === 'Aktif').length
      const totalTarget = data.reduce((sum, fund) => sum + fund.targetAmount, 0)
      const totalCollected = data.reduce((sum, fund) => sum + fund.currentAmount, 0)
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h1 style={{ fontSize: '2rem', fontWeight: '700', color: '#1a202c', margin: 0 }}>Fon Yönetimi</h1>
          <p style={{ color: '#64748b', margin: '0.5rem 0 0 0' }}>Bağış fonları ve kampanya yönetimi</p>
        </div>
      ...
      {/* Fon Listesi */}
      <div className="card">
        <h3 style={{ fontSize: '1.25rem', fontWeight: '600', color: '#1a202c', marginBottom: '1.5rem' }}>Fon Listesi</h3>
        
        {loading && (
          <div style={{ textAlign: 'center', padding: '2rem' }}>
            <div style={{ fontSize: '1rem', color: '#64748b' }}>Yükleniyor...</div>
          </div>
        )}
        ...
        {!loading && !error && filteredFunds.length === 0 && (
          <div style={{ textAlign: 'center', padding: '2rem' }}>
            <div style={{ fontSize: '1rem', color: '#64748b' }}>Fon bulunamadı</div>
          </div>
        )}
            ...
            return (
              ...
              >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ marginBottom: '1rem' }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.5rem' }}>
                        <span style={{ fontSize: '0.75rem', color: '#64748b' }}>%{progressPercentage.toFixed(1)} tamamlandı</span>
                        <span style={{ fontSize: '0.75rem', color: '#64748b' }}>Kalan: ₺{(fund.targetAmount - fund.currentAmount).toLocaleString()}</span>
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem' }}>
                      <div style={{ padding: '0.75rem', backgroundColor: '#f8fafc', borderRadius: '8px' }}>
                        <div style={{ fontSize: '0.75rem', color: '#64748b', marginBottom: '0.25rem' }}>Bağışçı Sayısı</div>
                        <div style={{ fontSize: '1rem', fontWeight: '600', color: '#1a202c', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                          <Users size={14} />
                          {fund.donorCount}
                        </div>
                      </div>
                      <div style={{ padding: '0.75rem', backgroundColor: '#f8fafc', borderRadius: '8px' }}>
                        <div style={{ fontSize: '0.75rem', color: '#64748b', marginBottom: '0.25rem' }}>Başlangıç</div>
                        <div style={{ fontSize: '1rem', fontWeight: '600', color: '#1a202c', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                          <Calendar size={14} />
                          {fund.startDate}
                        </div>
                      ...
                      <div style={{ padding: '0.75rem', backgroundColor: '#f8fafc', borderRadius: '8px' }}>
                        <div style={{ fontSize: '0.75rem', color: '#64748b', marginBottom: '0.25rem' }}>Bitiş</div>
                        <div style={{ fontSize: '1rem', fontWeight: '600', color: '#1a202c', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                          <Calendar size={14} />
                          {fund.endDate}
                        </div>
                      ...
                      <div style={{ padding: '0.75rem', backgroundColor: '#f8fafc', borderRadius: '8px' }}>
                        <div style={{ fontSize: '0.75rem', color: '#64748b', marginBottom: '0.25rem' }}>Ortalama Bağış</div>
                        <div style={{ fontSize: '1rem', fontWeight: '600', color: '#1a202c' }}>₺{Math.round(fund.currentAmount / fund.donorCount).toLocaleString()}</div>
                      </div>
                    </div>
                  </div>
                  <span
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  ...
                  <motion.button
                    onClick={() => handleUpdateFund(fund.id, { currentAmount: fund.currentAmount + 1000 })}
                    style={{
                      padding: '0.5rem 1rem',
