...
export const rateLimiters = {
  login: createRateLimiter(
    ...
    RATE_LIMIT_CONFIG.login.timeWindow
  ),
  api: createRateLimiter(
    RATE_LIMIT_CONFIG.api.maxRequests,
    RATE_LIMIT_CONFIG.api.timeWindow
  ),
  upload: createRateLimiter(
    RATE_LIMIT_CONFIG.upload.maxRequests,
    RATE_LIMIT_CONFIG.upload.timeWindow
  ),
  form: createRateLimiter(
    RATE_LIMIT_CONFIG.form.maxRequests,
    RATE_LIMIT_CONFIG.form.timeWindow
  ),
};
...
export const secureFileUpload = {
  ...
  validateFile(file, allowedCategories = ['image', 'document']) {
    ...
    const maxSize = Math.max(
      ...allowedCategories.map(cat => this.maxSizes[cat])
    );
    if (file.size > maxSize) {
      errors.push(
        `Dosya boyutu çok büyük. Maksimum ${Math.round(maxSize / 1024 / 1024)}MB olmalıdır.`
      );
    }

    // Check file type
    const allowedTypes = allowedCategories.flatMap(
      cat => this.allowedTypes[cat]
    );
