...
export const LineChart = ({ data, width = 400, height = 300, color = '#667eea' }) => {
  ...
  const getY = (value) => padding + (1 - (value - minValue) / (maxValue - minValue)) * chartHeight
...
// Bar Chart Component
export const BarChart = ({ data, width = 400, height = 300, color = '#667eea' }) => {
  if (!data || data.length === 0) return null
  ...
  return (
    <svg width={width} height={height}>
      ...
      {data.map((item, index) => (
        <text
          key={`label-${index}`}
          x={padding + (index * (chartWidth / data.length)) + (chartWidth / data.length) / 2}
          y={height - padding + 20}
          textAnchor="middle"
          fontSize="12"
          fill="#6b7280"
