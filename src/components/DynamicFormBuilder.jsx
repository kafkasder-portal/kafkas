...
const DynamicFormBuilder = () => {
  ...
  const fieldTypes = [
    ...
    { id: 'checkbox', name: 'Çoklu Seçim', icon: CheckSquare, color: '#ec4899' },
  ...
  const addElement = (type) => {
    const newElement = {
      ...
      placeholder: '',
      required: false,
      options: type === 'select' || type === 'radio' || type === 'checkbox' ? ['Seçenek 1', 'Seçenek 2'] : [],
      validation: {},
      style: {}
    }
  ...
  const renderFieldPreview = (element) => {
    ...
    switch (element.type) {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {element.options.map((option, idx) => (
              <label key={idx} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <input type="radio" name={`radio_${element.id}`} value={option} />
                {option}
              </label>
            ))}
          </div>
      ...
      default:
        return <input type="text" style={commonStyles} placeholder={element.placeholder} />
    }
  }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {(element.type === 'select' || element.type === 'radio' || element.type === 'checkbox') && (
            <div>
              ...
              <button
                onClick={() => {
                  const newOptions = [...element.options, `Seçenek ${element.options.length + 1}`]
                  updateElement(element.id, { options: newOptions })
                }}
                style={{
                  width: '100%',
    ...
    <div className="page-container">
      ...
      >
        <div>
          ...
          <p className="page-subtitle">Dinamik formlar oluşturun ve özelleştirin</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isPreviewMode ? '1fr' : '1fr 300px', gap: '2rem' }}>
        <div>
          {/* Form Header */}
          <motion.div 
            className="card"
          ...
          >
            {!isPreviewMode ? (
              ...
              <div>
                <h2 style={{ margin: 0, marginBottom: '0.5rem' }}>{formTitle}</h2>
                {formDescription && (
                  <p style={{ margin: 0, color: '#6b7280' }}>{formDescription}</p>
                )}
              </div>
            )}
          </motion.div>
              ...
              {formElements.length === 0 ? (
                ...
                >
                  ...
                  <p style={{ marginTop: '1rem', fontSize: '1.125rem' }}>
                    {isPreviewMode ? 'Form henüz boş' : 'Form alanları eklemek için yukarıdaki butonları kullanın'}
                  </p>
                </motion.div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                  {formElements.map((element, index) => {
                    const fieldType = fieldTypes.find(f => f.id === element.type)
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                          <label style={{
                            ...
                            {!isPreviewMode && <Icon size={16} color={fieldType?.color} />}
                            {element.label}
                            {element.required && <span style={{ color: '#dc2626' }}>*</span>}
                          </label>
        ...
        {!isPreviewMode && (
          <div>
            {selectedElement ? (
              <ElementEditor element={formElements.find(e => e.id === selectedElement)} />
            ) : (
              <motion.div
                className="card"
                style={{ position: 'sticky', top: '2rem' }}
