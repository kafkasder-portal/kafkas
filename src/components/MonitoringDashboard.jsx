...
// Hoisted helpers for reuse across inner components
...
const MonitoringDashboard = () => {
	...
	const performanceScores = useMemo(() => {
		...
		scores.overall = scoreValues.length > 0
...
const OverviewTab = ({ metrics, healthData, analyticsData, performanceScores, showDetails }) => {
	return (
		<div className="space-y-6">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
				...
				<PerformanceScoreCard
					...
					value={metrics.performance?.lcp ? formatTime(metrics.performance.lcp) : 'N/A'}
				/>
				<PerformanceScoreCard
					title="FID"
					...
					value={metrics.performance?.fid ? formatTime(metrics.performance.fid) : 'N/A'}
				/>
				<PerformanceScoreCard
					title="CLS"
					...
					value={metrics.performance?.cls ? metrics.performance.cls.toFixed(3) : 'N/A'}
				/>
			</div>
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
				...
				<AnalyticsOverviewCard analyticsData={analyticsData} showDetails={showDetails} />
			</div>
...
const PerformanceTab = ({ metrics, performanceScores, showDetails }) => {
	return (
		<div className="space-y-6">
					...
					{metrics.performance?.memory ? (
						<div className="space-y-4">
							...
							<div className="flex justify-between text-sm text-slate-500">
								<span>Toplam: {formatMemory(metrics.performance.memory.total * 1024 * 1024)}</span>
								<span>Limit: {formatMemory(metrics.performance.memory.limit * 1024 * 1024)}</span>
							</div>
						</div>
					) : (
			...
			{/* API Performance */}
			{showDetails && (
				<APIPerformanceCard metrics={metrics} />
			)}
		</div>
	)
}
				...
				<div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-6">
					...
					<div className="space-y-4">
						...
						<div className="flex justify-between">
							...
							<span className="font-semibold">
								{healthData.system?.uptime ? Math.floor(healthData.system.uptime / 60) : 0} dakika
							</span>
						</div>
						<div className="flex justify-between">
							<span>Bellek Kullanımı</span>
							<span className="font-semibold">
								{healthData.system?.memory ? `${healthData.system.memory.percentage}%` : 'N/A'}
							</span>
						</div>
					</div>
				</div>
				...
				<div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-6">
					...
					<div className="space-y-4">
						<div className="flex justify-between">
							...
							<span className={`font-semibold ${healthData.network?.status === 200 ? 'text-green-500' : 'text-red-500'
								}`}
							>
						...
						<div className="flex justify-between">
							...
							<span className="font-semibold">
								{healthData.network?.responseTime ? `${healthData.network.responseTime}ms` : 'N/A'}
							</span>
						</div>
						<div className="flex justify-between">
							...
							<span className="font-semibold">
								{healthData.network?.timestamp ?
									new Date(healthData.network.timestamp).toLocaleTimeString() : 'N/A'}
							</span>
						</div>
					</div>
				</div>
			...
			{showDetails && (
				<div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-6">
					...
					<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
						{Object.entries(healthData.application?.features || {}).map(([feature, status]) => (
...
const PerformanceScoreCard = ({ title, score, icon: Icon, description, value }) => {
	return (
		<motion.div
	...
	return (
		<div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-6">
			...
			<div className="flex items-center space-x-3 mb-4">
				...
				<span className={`font-semibold ${isHealthy ? 'text-green-500' : 'text-red-500'}`}>
			...
			{showDetails && (
				<div className="space-y-2 text-sm">
					...
					<div className="flex justify-between">
						<span>Ortam:</span>
						<span className="capitalize">{healthData.application?.environment || 'N/A'}</span>
					</div>
					<div className="flex justify-between">
						<span>Son Güncelleme:</span>
						<span>{healthData.application?.timestamp ?
							new Date(healthData.application.timestamp).toLocaleTimeString() : 'N/A'}</span>
					</div>
				</div>
			)}
		</div>
			...
			{showDetails && (
				<div className="space-y-2 text-sm">
					<div className="flex justify-between">
						...
						<span>{Math.floor(analyticsData.sessionDuration / 60)}m {analyticsData.sessionDuration % 60}s</span>
					</div>
					<div className="flex justify-between">
				...
				<div className="space-y-3">
					{recentErrors.length > 0 ? (
						recentErrors.map((error, index) => (
							<div key={index} className="text-sm p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
				...
				<div className="space-y-3">
					{recentAPICalls.length > 0 ? (
						recentAPICalls.map((call, index) => (
							<div key={index} className="text-sm p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
...
const APIPerformanceCard = ({ metrics }) => {
	const apiCalls = Object.entries(metrics.apiCalls || {})
		.map(([url, calls]) => ({
			...
			avgDuration: calls.reduce((sum, call) => sum + call.duration, 0) / calls.length,
			successRate: calls.filter(call => call.success).length / calls.length * 100
		}))
