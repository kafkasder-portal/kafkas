...
router.get('/', async (req, res) => {
  try {
    ...
    const countResult = await query(
      `SELECT COUNT(*) FROM hospital_referrals ${whereClause}`,
      queryParams
    );
    ...
    const referralsResult = await query(
      `SELECT id, patient_name, patient_phone, patient_age, patient_gender, 
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const referralResult = await query(
      `SELECT id, patient_name, patient_phone, patient_age, patient_gender, patient_address,
              medical_condition, urgency, hospital_name, department, doctor_name,
              appointment_date, status, referral_date, notes, estimated_cost,
              actual_cost, payment_status, insurance_info, emergency_contact,
              created_at, updated_at, created_by
       FROM hospital_referrals WHERE id = $1`,
      [id]
    );
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const existingReferral = await query(
      'SELECT id FROM hospital_referrals WHERE id = $1',
      [id]
    );
    ...
    await query(
      `UPDATE hospital_referrals SET ${updateFields.join(', ')} WHERE id = $${paramIndex}`,
      updateValues
    );

    // Get updated referral
    const updatedReferral = await query(
      `SELECT id, patient_name, patient_phone, patient_age, patient_gender, patient_address,
              medical_condition, urgency, hospital_name, department, doctor_name,
              appointment_date, status, referral_date, notes, estimated_cost,
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    await query(
      'UPDATE hospital_referrals SET status = $1, updated_at = NOW() WHERE id = $2',
      ['cancelled', id]
    );
