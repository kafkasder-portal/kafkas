...
router.get('/', async (req, res) => {
    try {
        const { page = 1, limit = 10, search = '', status = '', category = '', urgency = '', region = '' } = req.query;
        ...
        // Get total count
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) FROM beneficiaries ${whereClause}`, queryParams);
        const totalBeneficiaries = parseInt(countResult.rows[0].count);
        // Get beneficiaries with pagination
        const beneficiariesResult = await (0, database_1.query)(`SELECT id, first_name, last_name, phone, email, address, region, category, 
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const beneficiaryResult = await (0, database_1.query)(`SELECT id, first_name, last_name, phone, email, address, region, category,
       ...
       FROM beneficiaries WHERE id = $1`, [id]);
        ...
        const aidHistoryResult = await (0, database_1.query)(`SELECT id, aid_type, amount, description, aid_date, created_at
...
router.post('/', async (req, res) => {
    try {
        const { firstName, lastName, phone, email, address, region, category, urgency = 'medium', familySize, monthlyIncome, description, notes, registrationDate } = req.body;
        // Validation
        ...
        await (0, database_1.query)(`INSERT INTO beneficiaries (
...
router.put('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { firstName, lastName, phone, email, address, region, category, status, urgency, familySize, monthlyIncome, description, notes } = req.body;
        // Check if beneficiary exists
        const existingBeneficiary = await (0, database_1.query)('SELECT id FROM beneficiaries WHERE id = $1', [id]);
        ...
        await (0, database_1.query)(`UPDATE beneficiaries SET ${updateFields.join(', ')} WHERE id = $${paramIndex}`, updateValues);
        // Get updated beneficiary
        const updatedBeneficiary = await (0, database_1.query)(`SELECT id, first_name, last_name, phone, email, address, region, category,
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await (0, database_1.query)('UPDATE beneficiaries SET status = $1, updated_at = NOW() WHERE id = $2', ['inactive', id]);
        res.json({
            success: true,
...
router.post('/:id/aid', async (req, res) => {
    try {
        ...
        // Create aid record
        const aidId = (0, uuid_1.v4)();
        await (0, database_1.query)(`INSERT INTO aid_records (id, beneficiary_id, aid_type, amount, description, aid_date, created_at, created_by)
       VALUES ($1, $2, $3, $4, $5, $6, NOW(), $7)`, [aidId, id, aidType, amount, description, aidDate || new Date().toISOString().split('T')[0], null]);
        res.status(201).json({
            success: true,
