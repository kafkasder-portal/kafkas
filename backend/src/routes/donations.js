...
router.get('/', async (req, res) => {
    try {
        const { page = 1, limit = 10, search = '', type = '', status = '', startDate = '', endDate = '', minAmount = '', maxAmount = '' } = req.query;
        ...
        // Get total count
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) FROM donations ${whereClause}`, queryParams);
        const totalDonations = parseInt(countResult.rows[0].count);
        // Get donations with pagination
        const donationsResult = await (0, database_1.query)(`SELECT id, donor_name, donor_email, donor_phone, amount, currency, type, status, 
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const donationResult = await (0, database_1.query)(`SELECT id, donor_name, donor_email, donor_phone, amount, currency, type, status,
              description, donation_date, receipt_number, payment_method, notes,
              created_at, updated_at, created_by
       FROM donations WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
        const { donorName, donorEmail, donorPhone, amount, currency = 'TRY', type, description, donationDate, paymentMethod, notes } = req.body;
        // Validation
        ...
        await (0, database_1.query)(`INSERT INTO donations (
...
router.put('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { donorName, donorEmail, donorPhone, amount, currency, type, status, description, donationDate, paymentMethod, notes } = req.body;
        // Check if donation exists
        const existingDonation = await (0, database_1.query)('SELECT id FROM donations WHERE id = $1', [id]);
        ...
        await (0, database_1.query)(`UPDATE donations SET ${updateFields.join(', ')} WHERE id = $${paramIndex}`, updateValues);
        // Get updated donation
        const updatedDonation = await (0, database_1.query)(`SELECT id, donor_name, donor_email, donor_phone, amount, currency, type, status,
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await (0, database_1.query)('UPDATE donations SET status = $1, updated_at = NOW() WHERE id = $2', ['cancelled', id]);
        res.json({
            success: true,
